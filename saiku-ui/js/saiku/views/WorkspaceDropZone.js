...
var WorkspaceDropZone = Backbone.View.extend({
    ...
    events: {
        ...
        'click .d_measure>a.measureClose' : 'remove_measure_click',
        'click .d_level>a.yhnFilter': 'selections',
        'click .measure_fields>.dropdown' : 'measure_action',
        'click .axis_fields_header>.dropdown' : 'limit_axis',
        'click .clear_axis' : 'clear_axis',
        'click .hierarchy .yhnClose':'clear_single_axis',
        'click li span.closeSpan':'closeFilter'
    },
    ...
    initialize: function(args) {
        ...
        _.bindAll(this, "clear_axis","closeFilter");
    },
    ...
    closeFilter:function(event){
        ...
        var axes = myTarget.parentNode.getAttribute("axes");
        var hierarchy = myTarget.parentNode.getAttribute("hierarchy");
        var level = myTarget.parentNode.getAttribute("name")
        this.workspace.query.helper.removeSelection(axes,hierarchy,level);
    ...
    clear_single_axis:function(event){
        var self = this;
        event.preventDefault();
        var hierarchy = $(event.target).siblings(".level").attr('hierarchy');
        var level =  $(event.target).siblings(".level").attr('level');
        var levelLength =  $(event.target).parent().siblings();
        var levelNum = 0;
        for(var i=0;i<levelLength.length;i++){
            if(levelLength[i].style.display == "list-item"){
                levelNum++;
            }
        }
        if(levelNum!=0){
    ...
    selections: function(event, ui) {
        ...
        var $target = $(event.target).parent().first().hasClass('d_level') ?
            $(event.target).parent().first().find('.level') :
            $(event.target).parent().first();
	...
	measure_action: function(event) {
		...
		event.x =l+35;
        event.y = t - 7;
        $target.contextMenu(event);
        $target.find(".context-menu-root").append("<li class='arrow'></li>")
	},
             ...
             build: function($trigger, e) {
                ...
                var citems = {
                        ...
                        "grand_totals" : {name: "Grand totals", i18n: true, items:
                        {
                            ...
                            "show_totals_median": {name: "median", i18n: true},
                            "show_totals_var": {name: "var", i18n: true},
                            "show_totals_std": {name: "std", i18n: true}
                        }}
