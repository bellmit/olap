...
var WorkspaceToolbar = Backbone.View.extend({
    ...
    events: {
        'click a': 'call',
        'click .myseperatorButton':'exportToggle'
    },
    initialize: function(args) {
        ...
        _.bindAll(this, "call", "exportToggle","reflect_properties", "run_query",
    ...
    exportToggle:function(){
        if(!$("li.myseperatorButton").hasClass("disabled_toolbar")){
            if(document.querySelector(".myseperator").style.display == ""){
                document.querySelector(".myseperator").style.display = "block";
                document.querySelector("#yhnBackGroundShelter").style.display = "block";
            }else if(document.querySelector(".myseperator").style.display == "block"){
                document.querySelector(".myseperator").style.display = "none";
                document.querySelector("#yhnBackGroundShelter").style.display = "none";
            }else if(document.querySelector(".myseperator").style.display == "none"){
                document.querySelector(".myseperator").style.display = "block";
                document.querySelector("#yhnBackGroundShelter").style.display = "block";
            }
        }
    ...
    activate_buttons: function(args) {
        if (args !== null && args.data && args.data.cellset && args.data.cellset.length > 0 ) {
            ...
            $(args.workspace.el).find('li.disabled_toolbar').removeClass('disabled_toolbar');
    ...
    reflect_properties: function() {
        ...
        if (properties['saiku.olap.query.nonempty'] === true) {
            ...
            $(this.el).find('.non_empty').parent().css("background","#A4ADB4");
        }
        if (properties['saiku.olap.query.automatic_execution'] === true) {
            $(this.el).find('.auto').addClass('on');
            $(this.el).find('.auto').parent().css("background","#03A9F4");
        }
        ...
        if (this.workspace.query.getProperty('saiku.olap.result.formatter') !== "undefined" && this.workspace.query.getProperty('saiku.olap.result.formatter') == "flattened") {
            if (! $(this.el).find('.group_parents').hasClass('on')) {
                ...
                $(this.el).find('.group_parents').parent().css("background","#A4ADB4");
            }
        }
        ...
        if (Settings.MODE == 'VIEW' || this.workspace.isReadOnly) {
            ...
            /*view调整*/
            $(this.el).find(".myseperatorButton").css("right","246px");
            $(this.el).find(".yhnLittleSum").css("right","384px");
            $(this.el).find(".yhnEmpty").css("right","484px");
            $(this.el).find(".yhnRun").css("right","600px");
            $(this.el).find(".myseperator").css("right","247px");
            /*隐藏自动查询*/
            $(this.el).find(".yhnAutoRun").hide();
            /*check 按钮打开颜色是否正常*/
            if($(this.el).find(".auto").hasClass("on")){
                $(this.el).find(".auto").parent().css("background","#03A9F4");
            }
            if(!$(this.el).find(".non_empty").hasClass("on")){
                $(this.el).find(".non_empty").parent().css("background","#03A9F4");
            }
            if(!$(this.el).find(".group_parents").hasClass("on")){
                $(this.el).find(".group_parents").parent().css("background","#03A9F4");
            }
    ...
    automatic_execution: function(event) {
        ...
        if (newState) {
            ...
            $(event.target).parent().css("background","#03A9F4");
        } else {
            $(event.target).removeClass('on');
            $(event.target).parent().css("background","#A4ADB4");
        }
    },
    ...
    export_pdf: function(event) {
        ...
        this.workspace.chart.exportChart("jpg");
    },
    export_png:function(event){
        this.workspace.chart.exportChart("png");
    },
    export_jpg:function(event){
        this.workspace.chart.exportChart("jpg");
    },
    export_chart_pdf:function(event){
        this.workspace.chart.exportChart("pdf");
    },
    ...
    explain_query: function(event) {
        ...
        var explained = function(model, args) {
            var height = $(document).height() === $("body").height() ? $("body").height() : $(document).height();
            var explainPlan = "<textarea style='width: " + ($("body").width() - 165) + "px;height:" + (height - 175) + "px;'>";
