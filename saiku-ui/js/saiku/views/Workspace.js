...
var Workspace = Backbone.View.extend({
    ...
    events: {
        ...
        'click .cancel' : 'cancel',
        'click .admin' : 'admin',
        'click #yhnBackGroundShelter' :'backgroundShelter'
    },

    initialize: function(args) {
        // Maintain `this` in jQuery event handlers
        _.bindAll(this, "caption", "adjust","backgroundShelter", "toggle_sidebar", "prepare", "new_query",
    ...
    template: function() {
        ...
        if (this.isUrlCubeNavigation) {
            ...
            htmlCubeNavigation = this.selected_cube_template(1);
    ...
    adjust: function() {
        ...
        var heightReduction = 52;
        //�·������뱣�湤�����߶�
        var bottomReduction = 96;
        ...
        var height = $("body").height() > 100 ? $("body").height() : $(document).height();
        $separator.height(height - heightReduction);
         $(this.el).find('.sidebar').height(height - heightReduction);
        $(this.querytoolbar.el).find('div').height(height - heightReduction - 10);
        ...
        $(this.el).find('.workspace_results').css({
            height: height - heightReduction-bottomReduction
        });
        // 调整 全屏显示
        ...
        $bodyHeight = $('#header').length > 0 ? $('body').height() - $('#header').height() : $('body').height()
        
        var overHeight  = $('.workspace_inner').height() - $bodyHeight;
         if(overHeight > 0) {
          $targetWork = $('.workspace_results.ui-droppable')
          $targetWork.height($targetWork.height() - overHeight)
         }


        if (this.querytoolbar) { $(this.querytoolbar.el).find('a').tipsy({ delayIn: 1000, fade: true}); }
        if (this.toolbar) { $(this.toolbar.el).find('a').tipsy({ delayIn: 1000, fade: true}); }
    ...
    toggle_sidebar: function() {
        ...
        var calculatedMargin =
                (!$(this.el).find('.sidebar').hasClass('hide') ? 224 : 0) +
        ...
        $(this.el).find(".smallboy").toggleClass('hideSider');
    },
    prepare: function() {
        ...
        if (model.type === "QUERYMODEL") {
            ...
            if (!self.isReadOnly && (!Settings.hasOwnProperty('MODE') || (Settings.MODE != "table" && Settings.MODE != "view"))) {
                ...
                if (cms && cms.length > 0) {
                    ...
                    dimlist.find('.calculated_measures').html("");
                }
    ...
    update_parameters: function () {
        ...
        var paramDiv = "<span class='i18n'>参数</span>:";
    ...
    switch_view_state: function(mode, dontAnimate) {
        ...
        if (target == 'edit') {
                ...
                if ($(this.el).find('.sidebar').hasClass('hide')) {
                    this.toggle_sidebar();
                    this.query.helper.displayAllSelections();
                }
    ...
    block: function(message) {
        ...
        if(Settings.LOGO_32x32){
            ...
            $(self.el).block({
                message: '<div class="moduleGif" >&nbsp;&nbsp;</div> ' + message
            });
            Saiku.i18n.translate();
    ...
    error: function(args) {
        this.processing.html('<div class = "yhnErrorRow"><span></span><span>'+safe_tags_replace(args.data.error)+'</span></div>').show();
    }
});
