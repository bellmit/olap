...
var pvc = function(def, pv) {
        ...
        _addPropTooltip: function(ka) {
            if (!this.pvMark.hasTooltip) {
                ...
                if (tooltipFormatter) {
                    ...
                    this.pvMark.localProperty("tooltip").tooltip(this._createTooltipProp(tooltipFormatter, tipOptions.isLazy))
						.title(def.fun.constant("")).ensureEvents()
						.event(tipsyEvent, pv.Behavior.tipsy(tipOptions)).hasTooltip = !0;
                }
            }
        },
    ...
    def.type("pvc.visual.legend.BulletItemDefaultRenderer", pvc.visual.legend.BulletItemRenderer).init(function(keyArgs) {
        ...
        create: function(legendPanel, pvBulletPanel, extensionPrefix, wrapper) {
            ...
            if (drawRule) {
                ...
                }).lineWidth(10, pvc.extensionTag).strokeStyle(sceneColorProp, pvc.extensionTag), rp = this.rulePvProto;
    ...
    def.type("pvc.WaterfallPanel", pvc.BarAbstractPanel).add({
        ...
        _createCore: function() {
            this.base();
            var chart = this.chart, isVertical = this.isOrientationVertical(), anchor = isVertical ? "bottom" : "left",
				ao = this.anchorOrtho(anchor), ruleRootScene = this._buildRuleScene(), orthoScale = chart.axes.ortho.scale,
				orthoZero = orthoScale(0), sceneOrthoScale = chart.axes.ortho.sceneScale({
                sceneVarName: "value"
    ...
    def.type("pvc.BulletChartPanel", pvc.PlotPanel).add({
        ...
        _createCore: function(layoutInfo) {
            ...
            this.pvBulletTitle = this.pvBullet.anchor(anchor).add(pv.Label).font("bold 12px sans-serif").textAngle(angle).left(-10).textAlign(align).textBaseline("bottom").left(titleLeftOffset)
				.top(titleTopOffset).text(function(d) {
