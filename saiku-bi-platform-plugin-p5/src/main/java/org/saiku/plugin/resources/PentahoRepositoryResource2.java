...
public class PentahoRepositoryResource2 implements ISaikuRepository {
	...
	public List<IRepositoryObject> getRepository (
			final @QueryParam("path") String path,
			final @QueryParam("type") String type,
			@QueryParam("showHidden") Boolean hidden)
	{
...
//			}
			if (hidden == null) {
				...
				IUserSetting userSetting = settingsService.getUserSetting("MANTLE_SHOW_HIDDEN_FILES", String.valueOf(false));
				hidden = Boolean.parseBoolean(userSetting.getSettingValue());
			}
